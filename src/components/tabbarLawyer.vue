<template>
  <mt-tabbar v-model="message" fixed>
    ...
    <mt-tab-item id="lawyerMy">
      <img slot="icon" :src="messagecomputed=='lawyerMy'?this.atabs[3][1]:this.atabs[3][0]">
      我的
    </mt-tab-item>
  </mt-tabbar>
</template>
  ...
  export default {
      ...
      messagecomputed:{
        ...
        get(){
         let path = this.$route.path;
          ...
          {
            case 'lawyerIndex':
            case 'lawyerIndex/fTrial':
            case 'lawyerIndex/sTrial':
            case 'lawyerIndex/complete':
              this.message = 'lawyerIndex';      
          }
          return path.replace("/","");
        },
      }
    ...
    watch: {
      message: function (val, oldVal) {
        ...
        switch (val) {
          case 'lawyerIndex':
            if(this.$route.path!='/lawyerIndex'){
              this.$router.replace('/lawyerIndex');
            }
            break;
          case 'clueList':
          ...
          /*case 'soldiershall':
            ...
            break;*/
          case 'lawyerMy':
            if(this.$route.path!='/lawyerMy'){
              this.$router.replace('/lawyerMy');
            }
            break;
        }
      }
